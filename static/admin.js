...
async function atualizarStatus(pedidoId, novoStatus) {
    ...
    try {
        ...
        if (pedido) {
            ...
            atualizarEstatisticas();
            renderizarPedidos();
            alert(`Status atualizado localmente para "${novoStatus}" (modo demonstração - não salvo no servidor)`);
        } else {
            alert('Erro ao atualizar status. O servidor não está disponível.');
        }
        console.error(error);
    }
}
